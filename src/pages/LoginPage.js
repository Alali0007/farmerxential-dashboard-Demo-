...
export default function LoginPage({ onLogin }) {
  ...
  return (
    <div style={{
      ...
      position: 'relative',
      overflow: 'hidden',
      padding: '16px'  // padding so content never touches screen edges
    }}>

      {/* Grid background */}
      <div style={{
        position: 'absolute', inset: 0,
        backgroundImage: `linear-gradient(${C.green}18 1px, transparent 1px), linear-gradient(90deg, ${C.green}18 1px, transparent 1px)`,
        backgroundSize: '40px 40px'
      }}/>
      <div style={{
        position: 'absolute', inset: 0,
        background: 'radial-gradient(ellipse at center, #0D2B1E 0%, #060D0A 70%)'
      }}/>

      {/* Login box
          width: 100% on mobile, max 420px on desktop
          This is the key fix — instead of fixed 420px, we use maxWidth */}
      <div style={{
        ...
        borderRadius: 12,
        boxShadow: '0 0 60px rgba(27,67,50,0.4)',
        ...
        <div style={{ textAlign: 'center', marginBottom: 32 }}>
          ...
          <div style={{ color: C.gold, fontSize: 24, fontWeight: 'bold', letterSpacing: 3 }}>
            FARMERXENTIAL
          </div>
          <div style={{ color: C.dim, fontSize: 10, letterSpacing: 2, marginTop: 6 }}>
            IDENTIFYING THE FARMERS WHO NEED HELP MOST
          </div>
          <div style={{ color: '#1B4332', fontSize: 9, marginTop: 8, letterSpacing: 1 }}>
            BY LALISHANK HOLDINGS LIMITED
          </div>
        </div>

        {/* Fields */}
        <div style={{ color: C.dim, fontSize: 10, letterSpacing: 2, marginBottom: 6 }}>
          CLIENT_ID
        </div>
        <input
          value={name}
        ...
        {error && (
          <div style={{ color: C.high, fontSize: 12, marginBottom: 12, textAlign: 'center' }}>
            {error}
          </div>
        )}
